...
static int get_receiver_motion(double jd, char *tle_file, bool has_file, vec3 *llh_pos, object_motion_t *receiver)
{
    FILE * fp;
    tle_set * tle;
    if (receiver == NULL){
    ...
    if (tle_file != NULL){
        ...
        if (has_file) {
            ...
            tle = load_tle_from_string(tle_file);
            if (tle == NULL)
                return -1;
        }
...
static int propaget_single(propagation_config_t *conf, object_motion_t *object)
{
    ...
    if (conf->has_tle){
        if (get_receiver_motion(j_day(conf->timestamp), conf->tle, conf->has_tle_file, NULL, object) != 0){
            return -1;
        }
    }else{
        if (get_receiver_motion(j_day(conf->timestamp), NULL, 0, &conf->llh, object) != 0){
            return -1;
        }
    }
    return 0;
    ...
    if (propaget_single(&conf->station, &station) != -1) {
        ...
        if (propaget_single(&conf->platform, &platform) != -1) {
            ...
            if (get_payload_pointing(j_day(conf->station.timestamp), &platform, &station, &out) != -1) {
                ...
                res->dl_doppler =  1.0 * (conf->in_dl_freq - get_doppler(conf->in_dl_freq, res->rel_velocity));
                res->ul_doppler = -1.0 * (conf->in_ul_freq - get_doppler(conf->in_ul_freq, res->rel_velocity));
            }
        }
    }
    return ret;
